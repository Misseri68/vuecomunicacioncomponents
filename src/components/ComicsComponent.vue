<template>
    <div>
        <form v-on:submit.prevent="createComic()">
            ...
            <label> año </label>
            <input type="text" v-model="comicForm.year"/>
            <label> Imagen </label>
            <input type="text" v-model="comicForm.imagen"/>
            <button>Crear Comic</button>
        </form>

        <h1 style="margin-top: 2em;" v-if="favorito != null">Comic Favorito: {{ favorito.titulo }}</h1>

        <div style="margin-top: 2em;" id="comics" v-for="(comic, index) in comics" :key="comic">
            <ComicComponent id="comic" :index="index" :comic="comic" 
